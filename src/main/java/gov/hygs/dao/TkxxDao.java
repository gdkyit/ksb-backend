...
public class TkxxDao extends BaseJdbcDao {
	...
	public void insertUserResult(UserResult userRs){
		...
		this.jdbcTemplate.update(sql, new Object[]{
				userRs.getUserId(),userRs.getTkId(),userRs.getAnswer(),
				userRs.getResult(),userRs.getResultTime(),
				userRs.getResultScore(),userRs.getStartTime(),userRs.getEndTime()
		});
	}
	public void insertLaudRecord(LoudRecord rec){
		...
		this.jdbcTemplate.update(sql, new Object[]{
				rec.getUserId(),rec.getDeptId(),
				rec.getZstkId(),rec.getDzDate(),
	 ...
	 public void insertUser(Map<String,Object> user){
		   ...
		   this.jdbcTemplate.update(sql, new Object[]{
				   user.get("loginName"),user.get("userName")
				   ,user.get("phone"),user.get("deptId")
		   });
	   }
	 ...
	 /**
		...
		public List<Map<String,Object>> getDtxxRecordList(Integer userId){
			...
			sb.append("  select start_time,sum(score) as score,sum(right1) as rightCount,sum(error1) as errorCount,sum(right1+error1) totalCount  ");
			sb.append("  from(  ");
			sb.append("  select result_score as score,case when result ='Y'  then 1 when result='N' then 0 end as right1, case when result ='Y'  then 0 when result='N' then 1 end as error1,DATE_FORMAT(start_time,'%Y-%m-%d') as start_time  ");
		...
		public List<Map<String,Object>> getDtxxRecordDetail(Integer userId,String day){
			StringBuffer sb =  new StringBuffer(100);
			sb.append("  select tm.content,er.answer,er.result,er.result_SCORE as score from user_result as er,tktm as tm where  er.tm_id = tm.id_ ");
		...
		public Map<String,Object> getTotalUserResultByUserId(Integer userId){
			...
			sb.append(" 		 from exam_user_result as eur,exam_detail as ed,exam as e where eur.EXAM_DETAIL_ID = ed.id_ and ed.EXAM_ID = e.ID_ and e.EXAM_TYPE ='2'  ");
		...
		public List<Map<String,Object>> getNewTaskTm(Integer userId){
			Integer rowCount = this.getSystemProp().get("fxts" );
			String sql ="select tm.*,ly.TITLE as lytitle,ly.CONTENT as lycontent from tktm as tm ,tmly as ly where tm.TMLY_ID = ly.ID_ and tm.tmnd <>1 and tm.fl_id in (select tkfl_id from tkfldy where user_id = ? ) ORDER BY  RAND() LIMIT "+rowCount;
			return this.jdbcTemplate.queryForList(sql, new Object[]{userId});
		}
